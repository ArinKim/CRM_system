...
export class Sales {
  ...
  constructor(initializer?: any) {
    ...
    if (initializer.customer) {
      this.customer = new Customer(initializer.customer);
    }
    if (initializer.value) {
      this.value.amount = initializer.value.amount;
      this.value.currency = initializer.value.currency;
      this.value.date = new Date(initializer.value.date);
    }
  }
